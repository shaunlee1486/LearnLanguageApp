...
export default function GrammarTestPage() {
  ...
  const fetchTest = async () => {
    ...
    setError(null);
    try {
      const response = await api.get('/grammar/test/generate?count=10');
  ...
  const handleNext = async () => {
    if (currentIndex < questions.length - 1) {
      ...
      try {
        await api.post('/grammar/test/submit', {
          totalQuestions: questions.length,
          score: score + (isCorrect ? 1 : 0), // account for the last question if correct
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <p className="text-slate-400">Generating grammar test...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="max-w-2xl mx-auto pb-20">
        <Link href="/grammar" className="inline-flex items-center gap-2 text-slate-400 hover:text-slate-200 mb-6 transition-colors">
    ...
    return (
      <div className="max-w-xl mx-auto py-10 pb-20">
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 text-center shadow-2xl animate-in slide-in-from-bottom-8 duration-500">
          ...
          <p className="text-slate-400 mb-8">You finished your grammar session.</p>
  ...
  return (
    <div className="max-w-2xl mx-auto pb-20">
      <div className="flex items-center justify-between mb-8">
        <Link href="/grammar" className="inline-flex items-center gap-2 text-slate-400 hover:text-slate-200 transition-colors">
          <ArrowLeft className="w-4 h-4" />
          Quit Test
        </Link>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-xl mb-6">
        <h3 className="text-slate-400 uppercase tracking-widest text-sm font-semibold mb-6 flex items-center justify-between">
          <span>Which grammar rule does this sentence use?</span>
          <button 
        ...
        <div className="text-3xl font-bold text-slate-100 mb-6 italic">
          "{currentQ.exampleSentence}"
        </div>

        {showHint && (
          <div className="mb-6 p-4 bg-indigo-500/10 border border-indigo-500/20 rounded-xl text-indigo-200 animate-in fade-in slide-in-from-top-2">
            <strong>Hint:</strong> {currentQ.description}
          </div>
        )}
